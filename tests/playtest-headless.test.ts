...
import { createPlaytestRun } from "../src/game/playtest-fixtures";
import { makeCard, makeEnemy, resolve } from "../src/game/engine";
...
test("CLI malformed JSON recovers in protocol and batch reports errors with nonzero exit", () => {
  ...
  const batch = Bun.spawnSync(["bun", "scripts/playtest-cli.ts", "batch"], {
    stdin: Buffer.from(
