...
test("settings accept only the current format", () => {
  ...
  expect(settingsSchema.safeParse({ ...preferences, tutorial: false }).success).toBe(false);
...
test("gameplay speed defaults to 1 and accepts only the supported range", () => {
  ...
  for (const gameplaySpeed of [0.5, 1, 1.25, 2]) {
    ...
    expect(settingsSchema.parse(JSON.parse(JSON.stringify(preferences)))).toEqual(preferences);
  }
  for (const gameplaySpeed of [0, 0.49, 2.01, Infinity, NaN, "1", null]) {
    expect(settingsSchema.safeParse({ ...defaultSettings, gameplaySpeed }).success).toBe(false);
  }
  const { gameplaySpeed: _gameplaySpeed, ...missingSpeed } = defaultSettings;
