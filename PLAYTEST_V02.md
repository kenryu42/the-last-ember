...
```sh
bun scripts/playtest-cli.ts fight '{"rules":"control","fixture":{"deckId":"defense","variant":"base"},"encounterId":"ward","seed":"ember-v02-a","policy":"planner"}'

# Each input line is a complete fight config. Each output line is one result.
bun scripts/playtest-cli.ts batch < /tmp/fights.jsonl > /tmp/results.jsonl

# Persistent JSON-line session for an external playtester agent.
bun scripts/playtest-cli.ts protocol

# Frozen, staged campaign. Progress goes to stderr; one compact report to stdout.
bun scripts/playtest-campaign.ts > /tmp/ember-v02-campaign.json
```
...
Run `bun test tests/playtest.test.ts tests/playtest-headless.test.ts`.
...
```sh
bun scripts/playtest-cli.ts fight '{"rules":"candidate","fixture":{"deckId":"exposed","variant":"diagnostic-mixed"},"encounterId":"fury","seed":"ember-v02-recovery-005","policy":"offense"}'
bun scripts/playtest-mixed-campaign.ts > /tmp/ember-v02-mixed.json
jq '.cells[] | {encounter,policy,control,candidate,pairedCandidateMinusControl}' PLAYTEST_V02_MIXED_RESULTS.json
```

The follow-up predeclared `ember-v02-recovery-000` through `024`, all three encounters, four unchanged v1 policies and both rules: 600 policy fights. It also ran 600 cross-rule action replays, counted separately, not as additional policy samples. Budgets remained 40 turns, 400 actions, 96 speculative resolves and `belief-v1`. Runtime including replays was 38.04 seconds. `PLAYTEST_V02_MIXED_RESULTS.json` retains all paired summaries and six selected trace pairs with first-choice audits.
...
```sh
bun scripts/playtest-cli.ts fight '{"rules":"candidate","fixture":{"deckId":"exposed","variant":"diagnostic-mixed"},"encounterId":"ward","seed":"ember-v02-phase-004","policy":"planner-v2","planningSeed":"phase-study","searchBudget":256}'
bun scripts/playtest-phase-campaign.ts > /tmp/ember-v02-phase.json
jq '.ward8 | {fixedSequence,branches}' PLAYTEST_V02_PHASE_RESULTS.json
```
