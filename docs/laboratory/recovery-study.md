...
No gameplay files were edited by this assignment. The supervisor's 12 v0.2 files were copied first; that baseline passed 133 tests. Independent additions are `scripts/studies/recovery-study.ts`, `tests/lab/recovery-study.test.ts`, and this document. No CLI-worker files were changed, and nothing was pushed, published, or deployed.
...
```sh
bun test tests/lab/recovery-study.test.ts
bun test
bun run typecheck
bun run build
bun scripts/studies/recovery-study.ts > /tmp/recovery-results.json
bun scripts/studies/recovery-study.ts > /tmp/recovery-repeat.json
