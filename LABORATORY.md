...
`Action`. `src/game/content.ts` owns printed effects and enemy patterns.
`src/game/engine.ts` owns legality and all state transitions. `resolve` clones its
...
`scripts/lab.ts` provides CLI boundaries; `scripts/lab-report.py` analyzes JSONL.
...
bun scripts/lab.ts simulate --games 1 --seed demo --deck exposed --encounter fury --bot search > /tmp/one.jsonl

# A complete adventure, using the normal starter deck and normal adventure rules.
bun scripts/lab.ts journey --games 1 --seed journey --bot strategic

# Scales to 100, 1000, 10000 or more. Output is streamed, one result per line.
bun scripts/lab.ts simulate --games 10000 --seed campaign --bot greedy --quiet > /tmp/results.jsonl

# --games is per deck/encounter/bot cell for suites, not total games.
bun scripts/lab.ts suite --suite smoke --games 1 --seed smoke > /tmp/smoke.jsonl
bun scripts/lab.ts suite --suite balance --games 100 --seed lab-baseline-v1 > /tmp/balance.jsonl
bun scripts/lab.ts suite --suite stress --games 1120 --seed lab-stress-v1 > /tmp/stress.jsonl
bun scripts/lab.ts suite --suite exploit --games 40 --seed lab-exploit-v1 > /tmp/exploit.jsonl
bun scripts/lab.ts suite --suite regression --games 2 --seed regression > /tmp/regression.jsonl
bun scripts/lab.ts suite --suite experiments --games 50 --seed lab-experiment-v1 > /tmp/experiments.jsonl

# Machine summary with per-cell Wilson intervals; human-readable descriptive report.
bun scripts/lab.ts analyze < /tmp/balance.jsonl > /tmp/summary.json
python3 scripts/lab-report.py /tmp/balance.jsonl > /tmp/report.md
...
bun scripts/lab.ts replay --seed lab-baseline-v1:7 --deck quiet --encounter reinforce --bot search > /tmp/replay.json

# Reproduce the policy-only intervention and pair it against original Search.
bun scripts/lab.ts suite --suite smoke --bot search-tempo --games 100 --seed lab-baseline-v1 > /tmp/tempo.jsonl
python3 scripts/lab-report.py /tmp/balance.jsonl /tmp/tempo.jsonl --pair bot --control search --candidate search-tempo

# The experiments suite includes mixed-deck control/candidate pairs.
python3 scripts/lab-report.py /tmp/experiments.jsonl --pair rules --control control --candidate candidate
python3 scripts/lab-report.py /tmp/experiments.jsonl --pair variant --control base --candidate ablation
...
```sh
bun scripts/lab.ts journey --games 50 --seed lab-horizon-v2 --bot search --budget 256 > /tmp/horizon-search256.jsonl
bun scripts/lab.ts journey --games 50 --seed lab-horizon-v2 --bot search --budget 4096 > /tmp/horizon-search4096.jsonl
bun scripts/lab.ts journey --games 50 --seed lab-horizon-v2 --bot rollout-one --budget 4096 > /tmp/horizon-one.jsonl
bun scripts/lab.ts journey --games 50 --seed lab-horizon-v2 --bot search-two --budget 4096 > /tmp/horizon-two.jsonl
python3 scripts/lab-report.py --journeys /tmp/horizon-search256.jsonl /tmp/horizon-search4096.jsonl /tmp/horizon-one.jsonl /tmp/horizon-two.jsonl
bun test tests/laboratory-horizon.test.ts
```
...
`scripts/lab-chains.ts` constructs decks with every current card, one or three
...
for bot in search search-two search-sequence; do
  bun scripts/lab.ts journey --games 20 --seed lab-continuation-v3 --bot "$bot" --budget 32768 > "/tmp/v3-$bot.jsonl"
done
python3 scripts/lab-report.py --journeys /tmp/v3-*.jsonl

for bot in search search-two search-material; do
  bun scripts/lab.ts journey --games 30 --seed lab-utility-v4 --bot "$bot" --budget 4096 > "/tmp/v4-$bot.jsonl"
done
python3 scripts/lab-report.py --journeys /tmp/v4-*.jsonl

for progression in static shield-aware; do
  bun scripts/lab.ts journey --games 50 --seed lab-shield-v5 --bot search --budget 256 --progression "$progression" > "/tmp/v5-$progression.jsonl"
done
python3 scripts/lab-report.py --journeys /tmp/v5-*.jsonl
...
bun scripts/lab-checkpoint.ts /tmp/horizon-two.jsonl lab-horizon-v2:13 119 search-sequence 32768 > /tmp/marshal-sequence.json

bun scripts/lab-chains.ts --games 10000 --seed lab-chains-v1 > /tmp/chains.jsonl
for encounter in fury reinforce ward; do
  bun scripts/lab-chains.ts --games 3000 --seed lab-pool-fuzz-v2 --turns 10 --encounter "$encounter" > "/tmp/chains-$encounter.jsonl"
done
