...
import { labConfigSchema, simulate } from "../src/game/laboratory";
import {
  PLAYTEST_DECKS,
  PLAYTEST_ENCOUNTERS,
...
mkdirSync("experiments/identity", { recursive: true });
const path = `experiments/identity/${recordedLabel}-${prefix.replaceAll(/[^a-zA-Z0-9_-]/g, "_")}`;
writeFileSync(
  `${path}.jsonl.gz`,
